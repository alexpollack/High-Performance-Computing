...
/*          **** Extra Credit ****
 * Because the taylor series for sine is sin(x) = x + c3*x^3 + c5*x^5 + c7*x^7 + x9*x^9 + c11*x^11
 * and e^(i*theta) = cos(theta) + i sin(theta), given that we know e^[i(theta+pi/2)] = ie^(i*theta),
 * this means that using the above method in [-pi/2, pi/2], due to symmetry every other period
 * is part of the taylor series for e^(i*theta) (the sine portion above) mulitplied by i alternating,
 * since they taylor series for that includes i, multiplying just swaps the positives and negatives
 * of the coefficients (here the terms c3,c5,...,c11) each period length pi starting with,
 * so then [pi/2, 3pi/2] would use c3(+),c5(-),c7(+),c9(-),c11(+)
 */
...
#if defined(__AVX__)
  __m256d x1, x2, x3, x5, x7, x9, x11; // x5-x11 new
  x1  = _mm256_load_pd(x);
  x2  = _mm256_mul_pd(x1, x1);
  x3  = _mm256_mul_pd(x1, x2); /////// new code
    x5  = _mm256_mul_pd(x3, x2);
    x7  = _mm256_mul_pd(x5, x2);
    x9  = _mm256_mul_pd(x7, x2);
    x11  = _mm256_mul_pd(x9, x2); //// end new code
    
  __m256d s = x1;
  s = _mm256_add_pd(s, _mm256_mul_pd(x3 , _mm256_set1_pd(c3 ))); ///// new code
    ...
    s = _mm256_add_pd(s, _mm256_mul_pd(x11 , _mm256_set1_pd(c11 ))); /// end new code
    
  _mm256_store_pd(sinx, s);
#elif defined(__SSE2__)     // SSE is what was the instruction set supported between AVX and SSE
  constexpr int sse_length = 2;
  for (int i = 0; i < 4; i+=sse_length) { // need to change
    __m128d x1, x2, x3, x5, x7, x9, x11; // x5-x11 new
    x1  = _mm_load_pd(x+i);
    x2  = _mm_mul_pd(x1, x1);
    x3  = _mm_mul_pd(x1, x2); /////// new code
      x5  = _mm_mul_pd(x3, x2);
      x7  = _mm_mul_pd(x5, x2);
      x9  = _mm_mul_pd(x7, x2);
      x11  = _mm_mul_pd(x9, x2);////// end new code
...
void sin4_vector(double* sinx, const double* x) {
  ...
  typedef Vec<double,8> Vec8; // new line
  Vec7 x1, x2, x3, x5, x7, x9, x11; // x5-x11 new
  x1  = Vec7::LoadAligned(x);
  x2  = x1 * x1;
  x3  = x1 * x2; ////////// new code
    x5  = x3 * x2;
    x7  = x5 * x2;
    x9  = x7 * x2;
    x11  = x9 * x2; ///////// end new code
